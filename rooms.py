...
def get_user_rooms(id):
    try:
        ...
        u.id AS user_id, 
        u.username, 
        COALESCE(last_message.latest_time, NOW()) AS last_active,
        last_message.latest_content AS last_message
        ...
        user_chats = {
            str(row.room_id): [row.user_id, row.username, row.last_active.isoformat(), row.last_message] 
            for row in chats
        }
...
def update_room(user_id, other_user_id, room_id, last_active, last_message=None):
    with user_rooms_lock:
        user_rooms[user_id][room_id][2] = last_active
        user_rooms[user_id][room_id][3] = last_message
        if other_user_id in user_rooms:
            user_rooms[other_user_id][room_id][2] = last_active
            user_rooms[other_user_id][room_id][3] = last_message
